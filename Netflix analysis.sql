/* =========================================================
   ...
   - Check data quality
   - Validate time range & key fields
   ========================================================= */
...
-- 4. Date added range (convert string to date)
SELECT
    MIN(STR_TO_DATE(date_added, '%M %d, %Y')) AS first_date_added,
    MAX(STR_TO_DATE(date_added, '%M %d, %Y')) AS last_date_added
FROM netflix
...
-- 7. Top contributing countries (raw, before normalization)
SELECT
    country,
    COUNT(*) AS total_titles
FROM netflix
...
-- 8. Genre distribution (raw listed_in values)
SELECT
    listed_in AS genre_raw,
    COUNT(*) AS total_titles
FROM netflix
GROUP BY listed_in
ORDER BY total_titles DESC
...
-- 10. TV Shows season distribution (text-based check)
SELECT
    duration AS season_text,
    COUNT(*) AS total_titles
FROM netflix
WHERE type = 'TV Show'
GROUP BY duration
